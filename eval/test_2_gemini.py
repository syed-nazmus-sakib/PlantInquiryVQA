...
def run_test2_for_image(model_key: str, image_id: str, qa_pairs: List[Dict], 
...
def evaluate_model(model_key: str, image_data: Dict):
    """Evaluate a single model - runs in separate process."""
    
    try:
        config = MODEL_CONFIGS[model_key]
        output_dir = os.path.join(OUTPUT_BASE_DIR, config['output_dir'])
        os.makedirs(output_dir, exist_ok=True)
        
        gemini_client = genai.Client(api_key=GEMINI_API_KEY)
        rate_limiter = RateLimiter(config['rpm_limit'], config['tpm_limit'], config['rpd_limit'])
        
        print(f"\n[{model_key}] Starting TEST 2 evaluation")
        print(f"[{model_key}] Rate limits: RPM={config['rpm_limit']}, TPM={config['tpm_limit']}, RPD={config['rpd_limit']}")
        
        all_results, start_index = load_checkpoint(output_dir)
        if start_index > 0:
            print(f"[{model_key}] Resuming from image {start_index + 1}")
        
        image_items = list(image_data.items())
        pbar = tqdm(image_items[start_index:], initial=start_index, total=len(image_data), 
                   desc=f"{model_key}", position=list(MODEL_CONFIGS.keys()).index(model_key))
        
        for idx, (image_id, qa_pairs) in enumerate(pbar, start=start_index):
            pbar.set_description(f"{model_key} - {idx+1}/{len(image_data)}")
            
            image_path = os.path.join(IMAGE_DIR, image_id)
            
            if not os.path.exists(image_path):
                print(f"\n[{model_key}] WARNING: Image not found: {image_path}")
                continue
            
            try:
                result = run_test2_for_image(model_key, image_id, qa_pairs, image_path, 
                                            rate_limiter, gemini_client)
                all_results.append(result)
            except Exception as e:
                print(f"\n[{model_key}] ERROR processing {image_id}: {e}")
                traceback.print_exc()
        ...
        print(f"\n[{model_key}] Saving final results...")
        
        output_data = {
            'metadata': {
                'test_name': f'Test 2 - Question Scaffolding - {model_key}',
        ...
        print(f"\n[{model_key}] COMPLETED - Results saved to {output_dir}/results.json")
        print(f"[{model_key}] Total images: {len(all_results)}")
        print(f"[{model_key}] Total QA pairs: {sum(len(r['conditions']) for r in all_results)}")
        
        if os.path.exists(f"{output_dir}/checkpoint.json"):
            os.remove(f"{output_dir}/checkpoint.json")
            
    except Exception as e:
        print(f"\n[{model_key}] FATAL ERROR: {e}")
        traceback.print_exc()
...
def main():
    ...
    print("="*80)
    print("TEST 2: QUESTION SCAFFOLDING - GEMINI MODELS")
    print("="*80)
    print("\nTest Description:")
    ...
    print("\nStarting parallel evaluation processes...")
    print("Note: Each model will run in its own process")
    print("Results will contain only model answers and ground truth\n")
    
    processes = []
    
    for model_key in MODEL_CONFIGS.keys():
        print(f"Starting process for {model_key}...")
        p = Process(target=evaluate_model, args=(model_key, image_data))
        p.start()
        processes.append(p)
        time.sleep(2)
    
    print(f"\n{len(processes)} processes started. Waiting for completion...\n")
    
    for idx, p in enumerate(processes):
        model_key = list(MODEL_CONFIGS.keys())[idx]
        print(f"Waiting for {model_key} to complete...")
        p.join()
    ...
    print("\nResults summary:")
    for model_key in MODEL_CONFIGS.keys():
        output_dir = os.path.join(OUTPUT_BASE_DIR, MODEL_CONFIGS[model_key]['output_dir'])
        results_file = f"{output_dir}/results.json"
        if os.path.exists(results_file):
            with open(results_file, 'r') as f:
                data = json.load(f)
                print(f"\n{model_key}:")
                print(f"  Images processed: {data['metadata']['n_images']}")
                print(f"  Total QA pairs: {data['metadata']['total_qa_pairs']}")
                print(f"  Results file: {results_file}")
        else:
            print(f"\n{model_key}: Results file not found")
