...
# VERY CONSERVATIVE RATE LIMITS FOR GEMMA-3-27B-IT
# Chart shows: RPM=11/30, TPM=3.93K/15K, RPD=2.72K/14.4K
# Using the LOWER values to be safe
MODEL_CONFIG = {
    'model_name': 'gemma-3-27b-it',
    'rpm_limit': 10,  # Using 10 instead of 11 to be safe
    'tpm_limit': 3500,  # Using 3.5K instead of 3.93K to be safe
    'rpd_limit': 2500,  # Using 2.5K instead of 2.72K to be safe
    'output_dir': 'gemma3_27b'
}
...
class RateLimiter:
    ...
    def __init__(self, rpm_limit, tpm_limit, rpd_limit):
        ...
        # For low RPM, add significant delay between requests
        self.min_delay = 60.0 / rpm_limit if rpm_limit > 0 else 0.1
        # Add extra buffer for low rate limits
        if rpm_limit < 20:
            self.min_delay = self.min_delay * 1.3  # 30% more delay
    ...
    def wait_if_needed(self, estimated_tokens=500):
        ...
        # RPM limit check - be cautious
        if len(self.request_times) >= self.rpm_limit - 1:  # Leave 1 request buffer
            wait_time = 60 - (current_time - self.request_times[0])
            if wait_time > 0:
                print(f"\nApproaching RPM limit. Waiting {wait_time:.1f}s...")
                time.sleep(wait_time + 0.5)
                current_time = time.time()
                self.request_times = [t for t in self.request_times if current_time - t < 60]
        
        # TPM limit check - be cautious (this is very low!)
        total_tokens = sum(tokens for _, tokens in self.token_usage)
        if total_tokens + estimated_tokens > self.tpm_limit * 0.85:  # 85% threshold
            wait_time = 60 - (current_time - self.token_usage[0][0])
            if wait_time > 0:
                print(f"\nApproaching TPM limit ({total_tokens}/{self.tpm_limit}). Waiting {wait_time:.1f}s...")
                time.sleep(wait_time + 0.5)
                current_time = time.time()
                self.token_usage = [(t, tokens) for t, tokens in self.token_usage if current_time - t < 60]
        
        # RPD limit check - plenty of room here
        if len(self.daily_requests) >= self.rpd_limit - 10:  # Leave 10 request buffer
            wait_time = 86400 - (current_time - self.daily_requests[0])
            if wait_time > 0:
                print(f"\nApproaching daily limit ({len(self.daily_requests)}/{self.rpd_limit}). Waiting {wait_time/3600:.1f} hours...")
...
def main():
    ...
    print("\n⚠️  LOW RATE LIMITS - ESPECIALLY TPM")
    print(f"  - RPM: {MODEL_CONFIG['rpm_limit']} (conservative)")
    print(f"  - TPM: {MODEL_CONFIG['tpm_limit']} (VERY LOW - conservative)")
    print(f"  - RPD: {MODEL_CONFIG['rpd_limit']} (plenty of headroom)")
    ...
    total_requests = sum(qa_counts)
    min_per_request = 60.0 / MODEL_CONFIG['rpm_limit'] * 1.3  # With buffer
    estimated_hours = (total_requests * min_per_request) / 3600
    print(f"\n⏱️  Estimated time: ~{estimated_hours:.1f} hours for {total_requests} requests")
    print(f"   (With {MODEL_CONFIG['rpm_limit']} RPM limit + safety buffers)")
    print(f"   NOTE: Low TPM ({MODEL_CONFIG['tpm_limit']}) may cause additional delays")
    ...
    for idx, (image_id, qa_pairs) in enumerate(pbar, start=start_index):
        pbar.set_description(f"Processing {idx+1}/{len(image_data)} | Daily: {len(rate_limiter.daily_requests)}/{MODEL_CONFIG['rpd_limit']}")
