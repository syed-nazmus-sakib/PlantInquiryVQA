...
# VERY CONSERVATIVE RATE LIMITS FOR GEMINI-3-PRO
# Chart shows: RPM=7/25, TPM=7.66K/1M, RPD=251/250
# Using the LOWER values to be safe
MODEL_CONFIG = {
    'model_name': 'gemini-3-pro-preview',
    'rpm_limit': 5,  # Using 5 instead of 7 to be extra safe
    'tpm_limit': 7000,  # Using 7K instead of 7.66K to be safe
    'rpd_limit': 200,  # Using 200 instead of 251 to be safe
    'output_dir': 'gemini3_pro'
}
...
class RateLimiter:
    ...
    def __init__(self, rpm_limit, tpm_limit, rpd_limit):
        ...
        # For very low RPM, add significant delay between requests
        self.min_delay = 60.0 / rpm_limit if rpm_limit > 0 else 0.1
        # Add extra buffer for very low rate limits
        if rpm_limit < 10:
            self.min_delay = self.min_delay * 1.5  # 50% more delay
    ...
    def wait_if_needed(self, estimated_tokens=500):
        ...
        # RPM limit check - be extra cautious
        if len(self.request_times) >= self.rpm_limit - 1:  # Leave 1 request buffer
            wait_time = 60 - (current_time - self.request_times[0])
            if wait_time > 0:
                print(f"\nApproaching RPM limit. Waiting {wait_time:.1f}s...")
                time.sleep(wait_time + 1)  # Add 1 extra second
                current_time = time.time()
                self.request_times = [t for t in self.request_times if current_time - t < 60]
        
        # TPM limit check - be extra cautious
        total_tokens = sum(tokens for _, tokens in self.token_usage)
        if total_tokens + estimated_tokens > self.tpm_limit * 0.9:  # 90% threshold
            wait_time = 60 - (current_time - self.token_usage[0][0])
            if wait_time > 0:
                print(f"\nApproaching TPM limit. Waiting {wait_time:.1f}s...")
                time.sleep(wait_time + 1)
                current_time = time.time()
                self.token_usage = [(t, tokens) for t, tokens in self.token_usage if current_time - t < 60]
        
        # RPD limit check - be extra cautious
        if len(self.daily_requests) >= self.rpd_limit - 5:  # Leave 5 request buffer
...
def main():
    ...
    print("\n⚠️  VERY LOW RATE LIMITS - THIS WILL BE SLOW")
    print(f"  - RPM: {MODEL_CONFIG['rpm_limit']} (conservative)")
    print(f"  - TPM: {MODEL_CONFIG['tpm_limit']} (conservative)")
    print(f"  - RPD: {MODEL_CONFIG['rpd_limit']} (conservative)")
    ...
    min_per_request = 60.0 / MODEL_CONFIG['rpm_limit'] * 1.5  # With buffer
    ...
    print("\n" + "="*80)
    print("Starting evaluation... This will be VERY SLOW due to rate limits")
    print("="*80 + "\n")
